...
class SuperAgentRecentLoginsScreen extends StatefulWidget {
  final List<String> locations;

  const SuperAgentRecentLoginsScreen({
    super.key,
    required this.locations,
  });

  @override
  State<SuperAgentRecentLoginsScreen> createState() => _SuperAgentRecentLoginsScreenState();
}

class _SuperAgentRecentLoginsScreenState extends State<SuperAgentRecentLoginsScreen> {
  ...
  Future<void> _loadRecentLogins() async {
    ...
    try {
      final locationsParam = widget.locations.join(',');
    ...
    try {
      ...
      setState(() {
        ...
        if (result.containsKey('voucher_info') && result['voucher_info'] != null) {
          ...
          // Check if this voucher belongs to one of the superagent's locations
          final voucherLocation = voucherInfo['location']?.toString() ?? '';
          if (widget.locations.contains(voucherLocation)) {
            _filteredVouchers = [voucherInfo];
          } else {
            // User found but not in superagent's locations
            _filteredVouchers = [];
          }
        } else {
          // User not found
                    ...
                    : _filteredVouchers.isEmpty
                        ? EmptyState(
                            ...
                            subtitle: _searchQuery.isEmpty
                                ? 'No login activity in the last 24 hours across your locations.'
                                : _searchController.text.isNotEmpty
                                    ? 'No user found with username "${_searchController.text}" in your assigned locations'
