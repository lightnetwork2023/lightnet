...
class PayablesManagementScreen extends StatefulWidget {
  const PayablesManagementScreen({super.key});

  @override
  State<PayablesManagementScreen> createState() => _PayablesManagementScreenState();
}

class _PayablesManagementScreenState extends State<PayablesManagementScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Payables (We Owe)'),
        actions: [
  ...
  Widget _buildSummaryCard() {
    return StreamBuilder<QuerySnapshot>(
      stream: DebtService.payables.snapshots(),
      builder: (context, snapshot) {
        ...
        for (var doc in snapshot.data!.docs) {
          ...
          totalPaid += (data['amountPaid'] ?? 0.0).toDouble();
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(
                    child: _buildSummaryItem('Total Owed', totalOwed, AppTheme.errorColor),
                  ),
                  Expanded(
                    child: _buildSummaryItem('Paid', totalPaid, AppTheme.successColor),
                  ),
                  Expanded(
                    child: _buildSummaryItem('Balance', balance, AppTheme.warningColor),
                  ),
                ],
              ),
            ],
  ...
  Widget _buildDebtsList() {
    return StreamBuilder<QuerySnapshot>(
      stream: DebtService.payables.orderBy('createdAt', descending: true).snapshots(),
      builder: (context, snapshot) {
  ...
  Widget _buildDebtCard(String debtId, Map<String, dynamic> data) {
    final creditorName = data['creditorName'] ?? 'Unknown';
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildAmountDetail('Paid', amountPaid, AppTheme.successColor),
              ),
              Expanded(
                child: _buildAmountDetail('Balance', balance, AppTheme.errorColor),
              ),
            ],
          ),
          if (dueDate != null) ...[
            ...
            children: [
              PopupMenuButton<String>(
                ...
                onSelected: (value) {
                  switch (value) {
                    case 'pay':
                      _showRecordPaymentDialog(debtId, creditorName, balance);
                      break;
                    case 'history':
                      _showPaymentHistoryDialog(debtId, creditorName);
                      break;
                    case 'edit':
                      _showEditDialog(debtId, data);
                      break;
                    case 'delete':
                      _confirmDelete(debtId, creditorName);
                      break;
                  }
                },
                itemBuilder: (context) => [
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: const Text('Add New Debt (We Owe)'),
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: ['General', 'Supplier', 'Loan', 'Utilities', 'Rent', 'Equipment', 'Other']
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                if (creditorCtrl.text.isEmpty || amountCtrl.text.isEmpty) {
  ...
  void _showRecordPaymentDialog(String debtId, String creditorName, double balance) {
    final amountCtrl = TextEditingController();
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text('Record Payment to $creditorName'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  await DebtService.recordPayablePayment(
                    debtId: debtId,
                    amount: amount,
  ...
  void _showPaymentHistoryDialog(String debtId, String creditorName) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Payment History - $creditorName'),
        content: SizedBox(
          width: double.maxFinite,
          child: StreamBuilder<QuerySnapshot>(
            stream: DebtService.getPaymentHistory(debtId, true),
            builder: (context, snapshot) {
              if (!snapshot.hasData) {
                return const Center(child: CircularProgressIndicator());
              }
  ...
  void _showEditDialog(String debtId, Map<String, dynamic> data) {
    final creditorCtrl = TextEditingController(text: data['creditorName']);
        ...
        builder: (context, setState) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                try {
                  await DebtService.updatePayable(
                    debtId: debtId,
                    creditorName: creditorCtrl.text.trim(),
  ...
  void _confirmDelete(String debtId, String creditorName) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete Debt'),
        content: Text('Are you sure you want to delete debt to $creditorName?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
          ...
          ElevatedButton(
            onPressed: () async {
              try {
                await DebtService.deletePayable(debtId);
                if (context.mounted) {
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
